...
\t          생략하면 슬롯에 따라 기본값이 다르다 -
\t            정면(Up/Down) : 가운데 맨 아래 = ((width-1)*0.5, height-1)
\t            측면(Left/Right) : 이미지 한가운데 = ((width-1)*0.5, (height-1)*0.5)
\t          타일 영역이 회전하지 않기 때문에 이렇게 갈라야 충돌 영역과
\t          보이는 그림이 어긋나지 않는다.
...
def read_sprites(path):
    """원본을 읽어 OrderedDict{프롭 이름: OrderedDict{슬롯: 행 목록}}을 돌려준다."""
    ...
    while index < total_lines:
        matched = SPRITE_HEADER.match(lines[index].strip())
...
def write_xml(out_path, props, desc):
    with io.open(out_path, 'w', encoding='utf-8', newline=LF) as out_file:
        ...
        out_file.write(u'<PropSprites desc="%s">%s' % (desc, LF))

        for prop_name, slots in props.items():
            out_file.write(u'%s<Prop name="%s" tileSpan="1">%s' % (LF + TAB, prop_name, LF))

            for slot, rows in sorted_slots(slots):
                out_file.write(u'%s%s<Sprite dir="%s" width="%d" height="%d">%s'
                               % (TAB, TAB, slot, len(rows[0]), len(rows), LF))
...
def main():
    ...
    props = read_sprites(args.source)
    check_symbols(props)

    write_xml(args.output, props, desc)
    histogram = verify(args.output, props)
